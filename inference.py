...
# test()
###
base_model = LlamaForCausalLM.from_pretrained("/mnt/cachenew/sunyuhan/models/Llama-2-7b-chat-hf",
     torch_dtype=torch.float16,trust_remote_code=True).to(rank)
tokenizer = AutoTokenizer.from_pretrained("/mnt/cachenew/sunyuhan/models/Llama-2-7b-chat-hf")
dataset = load_dataset("json", data_files='/mnt/cachenew/sunyuhan/alpaca-lora/alpaca_data.json')
...
def create_dataset():
    ...
    if rank == 0:
        for li in gathered_li:
            final_li += li
        file_path = "/mnt/cachenew/sunyuhan/alpaca-lora/syh_short_alpaca.json"
        # Write the list of dictionaries to the JSON file
        import json
        with open(file_path, 'w') as json_file:
            json.dump(final_li, json_file)
...
def create_cot_dataset():
    ...
    if rank == 0:
        for li in gathered_li:
            final_li += li
        file_path = "/mnt/cachenew/sunyuhan/alpaca-lora/syh_cot_alpaca.json"
        # Write the list of dictionaries to the JSON file
        import json
        with open(file_path, 'w') as json_file:
            json.dump(final_li, json_file)
...
def create_qa_dataset():
    ...
    if rank == 0:
        ...
        for li in gathered_li:
            final_li += li
        file_path = "/mnt/cachenew/sunyuhan/alpaca-lora/wiki-trivia-qa-noref.json"
        # Write the list of dictionaries to the JSON file
        import json
        with open(file_path, 'w') as json_file:
            json.dump(final_li, json_file)
...
def qa():
    ...
    file_path = "/mnt/cachenew/sunyuhan/alpaca-lora/qa_exp100702.json"
    # Write the list of dictionaries to the JSON file
    import json
    with open(file_path, 'w') as json_file:
...
def lora():
    ...
    out_of_range = [i for i in range(-100,0)] + [i for i in range(101,200)]
    for alpha in out_of_range:
        alpha = alpha / 100
        model = PeftModel.from_pretrained(
            base_model,
            lora_weights,
    ...
    file_path = "/mnt/cachenew/sunyuhan/alpaca-lora/alpha_exp1204.json"
    # Write the list of dictionaries to the JSON file
    import json
    with open(file_path, 'w') as json_file:
        json.dump(results, json_file)
